...
import {
  ...
  Paperclip, Image, ChevronDown, TrendingUp, Star, Bookmark, CheckCircle, X,
} from "lucide-react";
...
export const INITIAL_POSTS: Post[] = [
  {
    id: 1, author: "Aria Sharma", initials: "AS", role: "Founder @ EcoGrid AI",
    badge: "Raising", badgeColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    time: "4m",
    ...
    tag: "Climate Tech · Seed", tagColor: "text-cyan-400 bg-cyan-400/8 border-cyan-400/20",
    likes: 142, comments: 18, reposts: 34, liked: false, verified: true,
  },
  {
    id: 2, author: "Marcus Lindholm", initials: "ML", role: "GP @ Linea Capital",
    badge: "Investor", badgeColor: "text-[#15A9FF] bg-[#15A9FF]/10 border-[#15A9FF]/20",
    time: "9m",
    ...
    tag: "Deep Tech · Series A Interest", tagColor: "text-violet-400 bg-violet-400/8 border-violet-400/20",
    likes: 287, comments: 41, reposts: 76, liked: true, verified: true,
  },
  {
    id: 3, author: "Priya Venkatesh", initials: "PV", role: "Partner @ Aurora Early",
    badge: "Investor", badgeColor: "text-[#15A9FF] bg-[#15A9FF]/10 border-[#15A9FF]/20",
    time: "22m",
    ...
    tag: "Venture · Opinion", tagColor: "text-amber-400 bg-amber-400/8 border-amber-400/20",
    likes: 503, comments: 67, reposts: 128, liked: false,
  },
  {
    id: 4, author: "Dev Patel", initials: "DP", role: "Co-founder @ FinFlow",
    badge: "Raising", badgeColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    time: "1h",
    ...
    tag: "FinTech · Series A", tagColor: "text-emerald-400 bg-emerald-400/8 border-emerald-400/20",
    image: "chart", likes: 94, comments: 23, reposts: 15, liked: false, verified: true,
  },
  {
    id: 5, author: "Sarah Chen", initials: "SC", role: "CEO @ MediSync",
    badge: "Portfolio", badgeColor: "text-violet-400 bg-violet-400/10 border-violet-400/20",
    time: "2h",
    ...
    tag: "HealthTech · Milestone", tagColor: "text-rose-400 bg-rose-400/8 border-rose-400/20",
    likes: 612, comments: 88, reposts: 201, liked: false, verified: true,
  },
];
    ...
    <div className="w-full rounded-xl overflow-hidden border border-white/10 bg-white/3 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5 text-emerald-400 text-sm font-semibold">
          <TrendingUp className="w-4 h-4" />+328%
        </div>
      </div>
...
// Global mini-toast used inside PostCard
function Snack({ msg, onDone }: { msg: string; onDone: () => void }) {
  return (
    <div
      className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-[#0d1f3c] border border-white/10 shadow-2xl text-xs text-white animate-in fade-in slide-in-from-bottom-4 pointer-events-auto"
      style={{ whiteSpace: "nowrap" }}
    >
      {msg}
      <button onClick={onDone}><X className="w-3 h-3 text-white/30" /></button>
    </div>
  );
}
...
export function PostCard({ post: initPost }: { post: Post }) {
  ...
  function handleRepost() {
    ...
    showSnack("Reposted to your network!");
  }
  ...
  function handleComment() {
    showSnack("Reply feature coming soon — upgrade to Pro to DM directly.");
  }

  return (
    <article className="border-b border-white/6 px-5 py-5 hover:bg-white/[0.02] transition-colors group">
      {snack && <Snack msg={snack} onDone={() => setSnack(null)} />}
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-white/80 leading-relaxed whitespace-pre-line mb-3">
            {displayText}
            {needsTruncate && (
              <button onClick={() => setExpanded((e) => !e)} className="ml-1 text-primary hover:underline text-sm">
                {expanded ? " Show less" : " Show more"}
              </button>
            )}
          </p>

          {post.image === "chart" && <div className="mb-3"><MiniChart /></div>}

          <div className="flex items-center gap-1 -ml-2">
            <button
              onClick={() => setPost((p) => ({ ...p, liked: !p.liked, likes: p.liked ? p.likes - 1 : p.likes + 1 }))}
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-white/40 hover:text-[#15A9FF] hover:bg-[#15A9FF]/8 transition-all text-xs font-medium group/btn"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-white/40 hover:text-emerald-400 hover:bg-emerald-400/8 transition-all text-xs font-medium group/btn"
            >
            ...
            <button
              onClick={handleShare}
              className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/8 transition-all text-xs font-medium group/btn"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-all ${
                post.bookmarked ? "text-amber-400" : "text-white/30 hover:text-amber-400 hover:bg-amber-400/8 opacity-0 group-hover:opacity-100"
              }`}
            >
...
export function FeedComposerAndPosts() {
  ...
  function submit() {
    if (!text.trim()) return;
    const newPost: Post = {
      id: Date.now(), author: "You", initials: "YO", role: "Member @ GoodMatter",
      time: "now", content: text.trim(), likes: 0, comments: 0, reposts: 0,
    };
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-white/6">
              <label className="p-1.5 rounded-lg text-white/30 hover:text-primary hover:bg-primary/10 transition-all cursor-pointer">
